...
import com.hbg.helper.RoomTypeBookingHelper.RoomTypeBookingHelper_1DB_1ExtraB_HBG
...
abstract class RoomTypeBookingTest_1DB_1ExtraB_HBG extends XSellBaseTest  {

	static final String Sheet_NAME_1DB_1ExtraBed_HBG = "1DB_1ExtraBed_HBG"
	...
	RoomTypeBookingHelper_1DB_1ExtraB_HBG helper = new RoomTypeBookingHelper_1DB_1ExtraB_HBG(EXCEL_SHEET, getClientId())
	...
	def "Verify RoomTypeBooking test #1 || #testDesc"(){
		...
		for(k in 0.. cities.size()-1){
			def fileContents_shpr = helper.searchHotelPriceRequestFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, cities[k])
		...
		where:
		rowNum | testDesc
		1 | "1DB_1ExtraB_HBG - SearchHotelPriceRequest"
	}
	...
	def "Verify RoomTypeBooking test 2 || #testDesc"(){
		...
		for (int i=0;i<hotelList.size();i++){
			...
			def fileContents = helper.hotelPriceBreakdownRequestFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, hDTO.getItemCity(), hDTO.getItemCode(), hDTO.getCategoryId())
			println "Search Request Xml.. \n${fileContents}"
		...
		if (flag){
			...
			def childActualValue = itemCityCode+"|"+itemCode
...
//			softAssert.assertTrue(childActualValue==actualValue, "Child RoomCategoryId : actual value not matching with expected value")
		...
		where:
		rowNum | testDesc
		1 | "1DB_1ExtraB_HBG - HotelPriceBreakdownRequest"
	}
	...
	def "Verify RoomTypeBooking test 3 || #testDesc"(){
		...
		for (int i=0;i<hotelList.size();i++){
			...
			def fileContents = helper.addBookingRequestFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, bookingReferenceNum, hDTO.getItemCity(), hDTO.getItemCode(), hDTO.getCategoryId())
			println "Search Request Xml.. \n${fileContents}"
		...
		where:
		rowNum | testDesc
		1 | "1DB_1ExtraB_HBG - AddBookingRequest"
	}
	...
	def "Verify RoomTypeBooking test 4 || #testDesc"(){
		...
		if (addBookingSuccess){
			...
			def fileContents = helper.searchBookingRequest1For_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum)
			println "Search Request Xml.. \n${fileContents}"
		...
		where:
		rowNum | testDesc
		1 | "1DB_1ExtraB_HBG - SearchBookingRequest With Date Range"
	}
	...
	def "Verify RoomTypeBooking test 5 || #testDesc"(){
		...
		if (addBookingSuccess){
			...
			def fileContents = helper.searchBookingRequest2For_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, successHotelDTO.getBookingId())
			println "Search Request Xml.. \n${fileContents}"
			node = RequestNewHelper.postHttpsXMLRequestWithHeaderNode(url,'application/xml',fileContents,'SearchBookingRequest',FITSiteIdHeaderKey,SITEID)
			responseBody = XmlUtil.serialize(node)
			logDebug("Search Response Xml..\n${responseBody}")
		...
		where:
		rowNum | testDesc
		1 | "1DB_1ExtraB_HBG - SBR with BookingId"
	}
	...
	def "Verify RoomTypeBooking test 6 || #testDesc"(){
		...
		if (addBookingSuccess){
			...
			def fileContents = helper.addBookingItemRequestFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, successHotelDTO.getBookingId(), successHotelDTO.getItemCity(), successHotelDTO.getItemCode(), successHotelDTO.getCategoryId())
			println "Search Request Xml.. \n${fileContents}"
		...
		where:
		rowNum | testDesc
		1 | "ABIR_TB_1Extra_1Cot_HBG - AddBookingItemRequest"
	}
	...
	def "Verify RoomTypeBooking test 7 || #testDesc"(){
		...
		if (addBookingSuccess){
			...
			def fileContents = helper.bookingItemPriceBreakdownFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, successHotelDTO.getBookingId())
			println "Search Request Xml.. \n${fileContents}"
			...
			def roomCategoryId  = node.ResponseDetails.BookingItemPriceBreakdownResponse.BookingItem.HotelItem.HotelRooms.HotelRoom.@Id[0]
			softAssert.assertTrue(roomCategoryId.equals(successHotelDTO.getCategoryId().toString()), "Room Category Id does not match")
		...
		where:
		rowNum | testDesc
		1 | "1DB_1ExtraB_HBG - BookingItemPriceBreakdown"
	}
	...
	def "Verify RoomTypeBooking test 8 || #testDesc"(){
		...
		if (addBookingSuccess){
			...
			def expectedValue
			boolean flag = false
			for (HotelDTO hDTO : gtaHotelList){
				
				def fileContents = helper.gtaAddBookingItemRequestFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, successHotelDTO.getBookingId(), hDTO.getItemCity(), hDTO.getItemCode(), hDTO.getCategoryId())
				println "AddBookingItemRequest Xml.. \n${fileContents}"
				node = RequestNewHelper.postHttpsXMLRequestWithHeaderNode(url,'application/xml',fileContents,'AddBookingRequest',FITSiteIdHeaderKey,SITEID)
				responseBody = XmlUtil.serialize(node)
				logDebug("AddBookingItem Response Xml..\n${responseBody}")
				...
				if ((responseBody.contains("ErrorId") && (responseBody.contains("Hotel is invalid"))) || !responseBody.contains("<BookingReference ReferenceSource=")){
					...
					for (j in 0.. respBookingRefsize) {
						if(respBookingRef[j] != null && respBookingRef[j].@ReferenceSource.equals("client")) {
							result=true
							softAssert.assertTrue(respBookingRef[j].text().equals(successHotelDTO.getBookingReference().toString()), "Booking reference number does not match")
						}
	
						if(respBookingRef[j] != null && respBookingRef[j].@ReferenceSource.equals("api")) {
							softAssert.assertTrue(respBookingRef[j].text().equals(successHotelDTO.getBookingId().toString()), "Booking ID does not match")
							break
						}
					}
					softAssert.assertTrue(result==true, "ReferenceSource is not client")
			...
			if (!flag){
				softAssert.assertTrue(false, "Failed to get valid response for all the hotels")
			}
	...
	def "Verify RoomTypeBooking test 9 || #testDesc"(){
		...
		if (addBookingSuccess){
			...
			def fileContents = helper.searchBookingItemFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, successHotelDTO.getBookingId().toString())
		...
		where:
		rowNum | testDesc
		1 | "1DB_1ExtraB_HBG - SearchBookingItem"
	}
	...
	def "Verify RoomTypeBooking test 10 || #testDesc"(){
		...
		if (addBookingSuccess){
			...
			def fileContents = helper.chargeConditionBookingFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, successHotelDTO.getBookingId())
		...
		where:
		rowNum | testDesc
		1 | "1DB_1ExtraB_HBG - ChargeCondition_Booking"
	}
	...
	def "Verify RoomTypeBooking test 11 || #testDesc"(){
		...
		if (addBookingSuccess){
			...
			def fileContents = helper.modifyBookingRequestFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, successHotelDTO.getBookingId())
		...
		where:
		rowNum | testDesc
		1 | "1DB_1ExtraB_HBG - MBR (change AgentRef)"
	}
	...
	def "Verify RoomTypeBooking test 12 || #testDesc"(){
		...
		if (addBookingSuccess){
			...
			def fileContents = helper.modifyBookingItemRequestFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, successHotelDTO.getBookingId(), successHotelDTO.getCategoryId())
...
//			String responseBody
//	
//			def fileContents = helper.cancelBookingItemRequestFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, successHotelDTO.getBookingId())
...
//		where:
//		rowNum | testDesc
//		1 | "1DB_1ExtraB_HBG - CancelBookingItemRequest"
//	}
//	
//	@Unroll
...
//		where:
//		rowNum | testDesc
//		1 | "1DB_1ExtraB_HBG - SearchBookingItem  (after 1 item cancelled)"
//	}
//	
//	@Unroll
...
//			String responseBody
//	
//			def fileContents = helper.cancelBookingRequestFor_1DB_1ExtraBed_HBG(Sheet_NAME_1DB_1ExtraBed_HBG, rowNum, successHotelDTO.getBookingId())
//			println "CancelBookingItemRequest Xml.. \n${fileContents}"
//			node = RequestNewHelper.postHttpsXMLRequestWithHeaderNode(url,'application/xml',fileContents,'CancelBookingItemRequest',FITSiteIdHeaderKey,SITEID)
//			responseBody = XmlUtil.serialize(node)
//			logDebug("CancelBookingItemRequest Response Xml..\n${responseBody}")
...
//		where:
//		rowNum | testDesc
//		1 | "1DB_1ExtraB_HBG - CancelBookingRequest"
//	}
}

class RoomTypeBookingTest_1DB_1ExtraB_HBG_C1237 extends RoomTypeBookingTest_1DB_1ExtraB_HBG {
...
class RoomTypeBookingTest_1DB_1ExtraB_HBG_C1238 extends RoomTypeBookingTest_1DB_1ExtraB_HBG {
	@Override
	String getClientId() {
		return "1238"
	}
...
class RoomTypeBookingTest_1DB_1ExtraB_HBG_C1239 extends RoomTypeBookingTest_1DB_1ExtraB_HBG {
	@Override
	String getClientId() {
		return "1239"
	}
...
class RoomTypeBookingTest_1DB_1ExtraB_HBG_C1244 extends RoomTypeBookingTest_1DB_1ExtraB_HBG {
	@Override
	String getClientId() {
		return "1244"
	}
...
class RoomTypeBookingTest_1DB_1ExtraB_HBGSingleClient extends RoomTypeBookingTest_1DB_1ExtraB_HBG {
